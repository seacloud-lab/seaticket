...
def _extract_imap_folder_name(list_line):
    line = _decode_imap_line(list_line)
...
def _send_imap_id(imap_conn, imap_host):
    host = (imap_host or '').lower()
    if not any(domain in host for domain in _IMAP_ID_REQUIRED_HOSTS):
        return
    try:
        ...
        logger.warning('Failed to send IMAP ID command to %s: %s', imap_host, e)


def _find_junk_folder(imap_conn):
    status, folders = imap_conn.list()
    if status == 'OK':
        for folder in folders or []:
            folder_str = _decode_imap_line(folder)
            if '\\Junk' in folder_str:
                folder_name = _extract_imap_folder_name(folder_str)
                if folder_name:
                    return folder_name

    # Fallback names for servers without SPECIAL-USE support.
    for name in ['Junk', 'Spam', 'Junk E-mail', 'Junk Mail', 'INBOX.Junk', 'INBOX.Spam']:
        quoted = _quote_imap_folder(name)
        try:
...
def move_emails_to_junk(config, message_ids):
    server_provider = config.get('server_provider', 'general_email_provider')
    if server_provider != 'general_email_provider':
        raise EmailConfigError('Only general_email_provider supports IMAP move to junk.')
    ...
    if not all([imap_host, imap_user, imap_password]):
        raise EmailConfigError('Email configuration is incomplete for IMAP move to junk.')

    normalized_message_ids = []
    for message_id in message_ids or []:
        value = str(message_id or '').strip()
        if value:
            normalized_message_ids.append(value)
    if not normalized_message_ids:
        return {'moved_count': 0, 'target_folder': ''}

    imap = None
    try:
        imap = imaplib.IMAP4_SSL(imap_host, int(imap_port), timeout=30)
        imap.login(imap_user, imap_password)
        _send_imap_id(imap, imap_host)

        junk_folder = _find_junk_folder(imap)
        if not junk_folder:
            raise EmailSendError('Unable to locate junk/spam folder.')
        quoted_junk_folder = _quote_imap_folder(junk_folder)

        status, _ = imap.select('INBOX')
        if status != 'OK':
            raise EmailSendError('Failed to open INBOX folder.')
        ...
        if capability_status == 'OK':
            capability_text = ' '.join(_decode_imap_line(item) for item in capability_data or [])
            supports_move = 'MOVE' in capability_text.upper()

        if supports_move:
            move_status, _ = imap.uid('MOVE', uid_csv, quoted_junk_folder)
            if move_status != 'OK':
                raise EmailSendError('IMAP MOVE command failed.')
        else:
            copy_status, _ = imap.uid('COPY', uid_csv, quoted_junk_folder)
            if copy_status != 'OK':
                raise EmailSendError('IMAP COPY command failed.')
            store_status, _ = imap.uid('STORE', uid_csv, '+FLAGS', '(\\Deleted)')
            if store_status != 'OK':
                raise EmailSendError('IMAP STORE command failed.')
            expunge_status, _ = imap.expunge()
            if expunge_status != 'OK':
                raise EmailSendError('IMAP EXPUNGE command failed.')
        ...
        raise EmailSendError('Failed to move emails to junk.')
    finally:
        if imap:
            try:
                imap.logout()
