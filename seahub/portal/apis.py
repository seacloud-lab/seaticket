...
class PortalIssuesView(APIView):
    ...
    def post(self, request, project_uuid):
        ...
        try:
            ...
            row = {
                get_column_name_from_schema('PortalIssuesTable', 'title'): title,
                get_column_name_from_schema('PortalIssuesTable', 'content'): content,
                get_column_name_from_schema('PortalIssuesTable', 'state'): portal_issue_state,
                get_column_name_from_schema('PortalIssuesTable', 'type'): type_name if type_name else None,
                get_column_name_from_schema('PortalIssuesTable', 'substate'): default_substate,
                get_column_name_from_schema('PortalIssuesTable', 'priority'): priority,
                get_column_name_from_schema('PortalIssuesTable', 'tags'): tag_ids,
                get_column_name_from_schema('PortalIssuesTable', 'creator'): username,
                get_column_name_from_schema('PortalIssuesTable', 'comment_count'): 0,
                get_column_name_from_schema('PortalIssuesTable', 'created_time'): now_datetime,
                get_column_name_from_schema('PortalIssuesTable', 'modified_time'): now_datetime,
                get_column_name_from_schema('PortalIssuesTable', 'deleted'): False,
            }
                ...
                try:
                    ...
                    if updated_content != content:
                        seadb_api.update_rows(project_uuid, portal_issues_table_name, [{
                            ...
                            'row': {
                                get_column_name_from_schema('PortalIssuesTable', 'content'): updated_content,
                            }
                        }])
                        row[get_column_name_from_schema('PortalIssuesTable', 'content')] = updated_content
                except Exception as e:
                    logger.error(e)
                    try:
    ...
    def put(self, request, project_uuid):
        ...
        for issue in results:
            ...
            if 'state' in row_data:
                ...
                updated_row[get_column_name_from_schema('PortalIssuesTable', 'state')] = issue_state_name
                if issue_state_name == 'closed':
                    updated_row[get_column_name_from_schema('PortalIssuesTable', 'closed_time')] = now_datetime
                elif issue_state_name == 'open':
                    updated_row[get_column_name_from_schema('PortalIssuesTable', 'closed_time')] = ''

            if 'substate' in row_data:
                updated_row[get_column_name_from_schema('PortalIssuesTable', 'substate')] = row_data.get('substate') or None

            if 'tags' in row_data:
                tags_value = row_data.get('tags')
                if tags_value is None:
                    updated_row[get_column_name_from_schema('PortalIssuesTable', 'tags')] = []
                else:
                    updated_row[get_column_name_from_schema('PortalIssuesTable', 'tags')] = [int(tag_id) for tag_id in tags_value]

            if 'type' in row_data:
                updated_row[get_column_name_from_schema('PortalIssuesTable', 'type')] = row_data.get('type') or None
            ...
            updated_row[get_column_name_from_schema('PortalIssuesTable', 'modified_time')] = now_datetime
            update_rows.append({
                'pk': issue.get('_pk'),
                'row': updated_row,
            })
    ...
    def delete(self, request, project_uuid):
        ...
        if update_rows:
            try:
                seadb_api.update_rows(project_uuid, get_table_name_from_schema('PortalIssuesTable', ), update_rows)
            except Exception as e:
                logger.error(e)
                error_msg = 'Internal Server Error'
                return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
...
class PortalIssueView(APIView):
    ...
    def put(self, request, project_uuid, issue_id):
        ...
        try:
            update_row = {}
            if title:
                update_row[get_column_name_from_schema('PortalIssuesTable', 'title')] = title
            if content:
                update_row[get_column_name_from_schema('PortalIssuesTable', 'content')] = content
            if is_update_type:
                update_row[get_column_name_from_schema('PortalIssuesTable', 'type')] = type_name
            if is_update_substate:
                update_row[get_column_name_from_schema('PortalIssuesTable', 'substate')] = substate_option_name
            if is_update_tags:
                update_row[get_column_name_from_schema('PortalIssuesTable', 'tags')] = tags
            if is_update_priority:
                update_row[get_column_name_from_schema('PortalIssuesTable', 'priority')] = priority
            now_datetime = datetime.datetime.now(datetime.UTC).isoformat()
            if issue_state_name or issue_state_name == '':
                issue_state_name = issue_state_name.lower()
                update_row[get_column_name_from_schema('PortalIssuesTable', 'state')] = issue_state_name
                if issue_state_name == 'closed':
                    update_row[get_column_name_from_schema('PortalIssuesTable', 'closed_time')] = now_datetime
                elif issue_state_name == 'open':
                    update_row[get_column_name_from_schema('PortalIssuesTable', 'closed_time')] = ''
            ...
            update_row[get_column_name_from_schema('PortalIssuesTable', 'modified_time')] = now_datetime
            update_rows = [
                {
                    'pk': issue.get('_pk'),
                    'row': update_row
    ...
    def delete(self, request, project_uuid, issue_id):
        ...
        try:
            ...
            seadb_api.update_rows(project_uuid, get_table_name_from_schema('PortalIssuesTable', ), [update_row])
        except Exception as e:
            logger.error(e)
            error_msg = 'Internal Server Error'
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
...
class PortalIssueCommentsView(APIView):
    ...
    def post(self, request, project_uuid, issue_id):
        ...
        try:
            now_datetime = datetime.datetime.now(datetime.UTC).isoformat()
            row = {
                get_column_name_from_schema('PortalIssueCommentsTable', 'issue_id'): issue.get('_pk'),
                get_column_name_from_schema('PortalIssueCommentsTable', 'creator'): username,
                get_column_name_from_schema('PortalIssueCommentsTable', 'content'): content,
                get_column_name_from_schema('PortalIssueCommentsTable', 'created_time'): now_datetime,
                get_column_name_from_schema('PortalIssueCommentsTable', 'modified_time'): now_datetime,
                get_column_name_from_schema('PortalIssueCommentsTable', 'deleted'): False,
            }
            portal_issue_comment_table_name = get_table_name_from_schema('PortalIssueCommentsTable', )
            ...
            seadb_api.update_rows(project_uuid, get_table_name_from_schema('PortalIssuesTable', ), [update_issue])
        except Exception as e:
            logger.error(e)
            error_msg = 'Internal Server Error'
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
...
class PortalIssueCommentView(APIView):
    ...
    def put(self, request, project_uuid, issue_id, comment_id):
        ...
        try:
            ...
            seadb_api.update_rows(project_uuid, get_table_name_from_schema('PortalIssueCommentsTable', ), [issue_comment_update])
        except Exception as e:
            logger.error(e)
            error_msg = 'Internal Server Error'
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
        ...
        try:
            ...
            seadb_api.update_rows(project_uuid, get_table_name_from_schema('PortalIssuesTable', ), [issue_update])
        except Exception as e:
            logger.error(e)
    ...
    def delete(self, request, project_uuid, issue_id, comment_id):
        ...
        try:
            ...
            seadb_api.update_rows(project_uuid, get_table_name_from_schema('PortalIssueCommentsTable', ), [update_issue_comment])
    ...
    def get(self, request, project_uuid):
        ...
        try:
            ...
            portal_issue_column_name_to_return_name = {
                get_column_name_from_schema('PortalIssuesTable', 'substate'): 'substates',
                get_column_name_from_schema('PortalIssuesTable', 'type'): 'types',
                get_column_name_from_schema('PortalIssuesTable', 'state'): 'states'
            }
...
class PortalIssueTrashAPIView(APIView):
    ...
    def delete(self, request, project_uuid):
        ...
        try:
            ...
            for issue_id in need_delete_ids:
                comment_sql = f"DELETE FROM `{get_table_name_from_schema('PortalIssueCommentsTable', )}` WHERE `issue_id` = {int(issue_id)}"
