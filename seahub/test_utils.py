...
class Fixtures(Exam):
    ...
    @fixture
    def group_with_workspace(self):
        return self.create_group_with_workspace(group_name='test_group_with_workspace', username=self.user.username)
    ...
    def create_group_with_workspace(self, **kwargs):
        group_name = kwargs['group_name']
        username = kwargs['username']
        group_id = ccnet_threaded_rpc.create_group(group_name, username)
        owner = '%s@seafile_group' % group_id
        self.group_workspace = create_repo_and_workspace(owner, -1)
        return ccnet_threaded_rpc.get_group(group_id)
    ...
    def remove_group_with_workspace(self, group_id=None):
        if not group_id:
            group_id = self.group_with_workspace.id
        owner = '%s@seafile_group' % group_id
