...
TICKET_EVENT_IGNORED_FIELDS = frozenset({
    get_seadb_column_name('TicketsTable', 'comment_count'),
    get_seadb_column_name('TicketsTable', 'modified_time'),
    get_seadb_column_name('TicketsTable', 'closed_time'),
})


def _format_ticket_event_value(field_name, field_value, tag_id_to_name=None):
    if field_name == get_seadb_column_name('TicketsTable', 'tags') and isinstance(field_value, list):
        return [
...
def build_ticket_data_event(event_type, old_row=None, new_row=None, seadb_api=None, project_uuid=None):
    ...
    tag_ids = []
    if get_seadb_column_name('TicketsTable', 'tags') in new_row:
        old_tags = old_row.get(get_seadb_column_name('TicketsTable', 'tags')) or []
        new_tags = new_row.get(get_seadb_column_name('TicketsTable', 'tags')) or []
...
class TicketsAPIView(APIView):
    ...
    def post(self, request, project_uuid):
        ...
        try:
            ...
            row = {
                get_seadb_column_name('TicketsTable', 'title'): title,
                get_seadb_column_name('TicketsTable', 'content'): content,
                get_seadb_column_name('TicketsTable', 'state'): ticket_state,
                get_seadb_column_name('TicketsTable', 'type'): type_name,
                get_seadb_column_name('TicketsTable', 'substate'): default_substate,
                get_seadb_column_name('TicketsTable', 'priority'): priority,
                get_seadb_column_name('TicketsTable', 'assignees'): assignees,
                get_seadb_column_name('TicketsTable', 'participants'): [username],
                get_seadb_column_name('TicketsTable', 'tags'): [int(tag_id) for tag_id in tag_ids],
                get_seadb_column_name('TicketsTable', 'creator'): username,
                get_seadb_column_name('TicketsTable', 'comment_count'): 0,
                get_seadb_column_name('TicketsTable', 'created_time'): now_datetime,
                get_seadb_column_name('TicketsTable', 'modified_time'): now_datetime,
                get_seadb_column_name('TicketsTable', 'deleted'): False,
                get_seadb_column_name('TicketsTable', 'due_date'): due_date,
            }
            if linked_connection_records is not None:
                # keep stored value as list[str]
                row[get_seadb_column_name('TicketsTable', 'linked_connection_records')] = list(set(linked_connection_records or []))
                ...
                try:
                    ...
                    if updated_content != content:
                        seadb_api.update_rows(project_uuid, TABLE_TICKETS, [{
                            'pk': int(ticket_pk),
                            'row': {
                                get_seadb_column_name('TicketsTable', 'content'): updated_content,
                            }
                        }])
                        row[get_seadb_column_name('TicketsTable', 'content')] = updated_content
                except Exception as e:
                    logger.error(e)
                    try:
            ...
            if linked_connection_records:
                ticket_link_diff = {
                    int(ticket_pk): (set(row.get(get_seadb_column_name('TicketsTable', 'linked_connection_records')) or []), set())
                }
                try:
    ...
    def put(self, request, project_uuid):
        ...
        for r in results:
            old_lcr_by_ticket_id[int(r.get('_pk'))] = r.get(get_seadb_column_name('TicketsTable', 'linked_connection_records')) or []
        ...
        for row in results:
            ...
            if 'state' in row_data:
                ...
                updated_row[get_seadb_column_name('TicketsTable', 'state')] = ticket_state_name
                if ticket_state_name == 'closed':
                    event_type = DataEventType.TICKET_CLOSED.value
                    updated_row[get_seadb_column_name('TicketsTable', 'closed_time')] = now_datetime
                elif ticket_state_name == 'open':
                    event_type = DataEventType.TICKET_REOPENED.value
                    updated_row[get_seadb_column_name('TicketsTable', 'closed_time')] = ''
            if 'substate' in row_data:
                updated_row[get_seadb_column_name('TicketsTable', 'substate')] = row_data.get('substate')
            if 'tags' in row_data:
                tags_value = row_data.get('tags')
                if tags_value is None:
                    updated_row[get_seadb_column_name('TicketsTable', 'tags')] = []
                else:
                    updated_row[get_seadb_column_name('TicketsTable', 'tags')] = [int(tag_id) for tag_id in tags_value]
            if 'type' in row_data:
                updated_row[get_seadb_column_name('TicketsTable', 'type')] = row_data.get('type')
            if 'content' in row_data:
                ...
                if link_urls and isinstance(link_urls, list):
                    file_urls = (file_urls or []) + link_urls
                updated_row[get_seadb_column_name('TicketsTable', 'content')] = content
            if 'linked_connection_records' in row_data:
                new_lcr = row_data.get('linked_connection_records', [])
                if new_lcr in (None, ''):
                    updated_row[get_seadb_column_name('TicketsTable', 'linked_connection_records')] = []
                    ...
                    try:
                        ...
                        return api_error(status.HTTP_400_BAD_REQUEST, str(e))
                    updated_row[get_seadb_column_name('TicketsTable', 'linked_connection_records')] = list(set(new_lcr))
                else:
                    error_msg = 'linked_connection_records invalid.'
                    return api_error(status.HTTP_400_BAD_REQUEST, error_msg)
                ...
                old_set = set(old_lcr)
                new_set = set(updated_row.get(get_seadb_column_name('TicketsTable', 'linked_connection_records')) or [])
            ...
            updated_row[get_seadb_column_name('TicketsTable', 'modified_time')] = now_datetime
            update_rows.append(
                {
                    'pk': row.get('_pk'),
                    'row': updated_row,
...
class TicketAPIView(APIView):
    ...
    def get(self, request, project_uuid, ticket_id):
        ...
        try:
            ...
            linked_connection_records = ticket.get(get_seadb_column_name('TicketsTable', 'linked_connection_records')) or []
    ...
    def put(self, request, project_uuid, ticket_id):
        ...
        is_update_participants = get_seadb_column_name('TicketsTable', 'participants') in request.data
        participants = request.data.get(get_seadb_column_name('TicketsTable', 'participants')) or '[]'
        ...
        if is_update_linked_connection_records:
            ...
            old_linked_connection_records = ticket.get(get_seadb_column_name('TicketsTable', 'linked_connection_records')) or []
        ...
        try:
            ...
            event_type = DataEventType.TICKET_UPDATED.value
            if title:
                update_row[get_seadb_column_name('TicketsTable', 'title')] = title
            if content:
                update_row[get_seadb_column_name('TicketsTable', 'content')] = content
            if is_update_type:
                update_row[get_seadb_column_name('TicketsTable', 'type')] = type_name
            if is_update_substate:
                update_row[get_seadb_column_name('TicketsTable', 'substate')] = substate_option_name
            if is_update_tags:
                update_row[get_seadb_column_name('TicketsTable', 'tags')] = tags
            if is_update_priority:
                update_row[get_seadb_column_name('TicketsTable', 'priority')] = priority
            if is_update_assignees:
                update_row[get_seadb_column_name('TicketsTable', 'assignees')] = assignees
            if is_update_due_date:
                update_row[get_seadb_column_name('TicketsTable', 'due_date')] = due_date or ''
            if is_update_linked_connection_records:
                update_row[get_seadb_column_name('TicketsTable', 'linked_connection_records')] = new_linked_connection_records
            if not is_update_participants:
                participants = ticket.get(get_seadb_column_name('TicketsTable', 'participants')) or []
                if username not in participants:
                    participants.append(username)
            now_datetime = datetime.datetime.now(datetime.UTC).isoformat()
            if ticket_state_name or ticket_state_name == '':
                ticket_state_name = ticket_state_name.lower()
                update_row[get_seadb_column_name('TicketsTable', 'state')] = ticket_state_name
                if ticket_state_name == 'closed':
                    event_type = DataEventType.TICKET_CLOSED.value
                    update_row[get_seadb_column_name('TicketsTable', 'closed_time')] = now_datetime
                elif ticket_state_name == 'open':
                    event_type = DataEventType.TICKET_REOPENED.value
                    update_row[get_seadb_column_name('TicketsTable', 'closed_time')] = ''
            ...
            if changes:
                ...
                for activity in new_activities:
                    ...
                    if field_name == 'state_substate':
                        state_column = get_column_from_columns_by_name(metadata, get_seadb_column_name('TicketsTable', 'state'))
                        substate_column = get_column_from_columns_by_name(metadata, get_seadb_column_name('TicketsTable', 'substate'))
                        state_key = (state_column or {}).get('key') or get_seadb_column_name('TicketsTable', 'state')
                        substate_key = (substate_column or {}).get('key') or get_seadb_column_name('TicketsTable', 'substate')
                        old_value = activity.get('old_value') or {}
                        new_value = activity.get('new_value') or {}
                        if isinstance(old_value, dict):
                            activity['old_value'] = {
                                state_key: get_option_id_by_name(
                                    metadata, get_seadb_column_name('TicketsTable', 'state'), old_value.get('state'),
                                    case_insensitive=True
                                ),
                                substate_key: get_option_id_by_name(
                                    metadata, get_seadb_column_name('TicketsTable', 'substate'), old_value.get('substate')
                                )
                            }
                        if isinstance(new_value, dict):
                            activity['new_value'] = {
                                state_key: get_option_id_by_name(
                                    metadata, get_seadb_column_name('TicketsTable', 'state'), new_value.get('state'),
                                    case_insensitive=True
                                ),
                                substate_key: get_option_id_by_name(
                                    metadata, get_seadb_column_name('TicketsTable', 'substate'), new_value.get('substate')
                                )
                            }
                    elif field_name in (
                        get_seadb_column_name('TicketsTable', 'state'),
                        get_seadb_column_name('TicketsTable', 'substate'),
                        get_seadb_column_name('TicketsTable', 'type'),
                    ):
                        # keep storage as option name; only return ids for frontend consistency
                        case_insensitive = field_name == get_seadb_column_name('TicketsTable', 'state')
...
class TicketCommentsAPIView(APIView):
    ...
    def post(self, request, project_uuid, ticket_id):
        ...
        try:
            now_datetime = datetime.datetime.now(datetime.UTC).isoformat()
            row = {
                get_seadb_column_name('TicketCommentsTable', 'ticket_id'): ticket.get('_pk'),
                get_seadb_column_name('TicketCommentsTable', 'creator'): username,
                get_seadb_column_name('TicketCommentsTable', 'content'): content,
                get_seadb_column_name('TicketCommentsTable', 'created_time'): now_datetime,
                get_seadb_column_name('TicketCommentsTable', 'modified_time'): now_datetime,
                get_seadb_column_name('TicketCommentsTable', 'deleted'): False,
                get_seadb_column_name('TicketCommentsTable', 'via_agent'): False,
            }
...
class TicketActivitiesAPIView(APIView):
    ...
    def get(self, request, project_uuid, ticket_id):
        ...
        for a in activities:
            ...
            elif field_name in (get_seadb_column_name('TicketsTable', 'state'), get_seadb_column_name('TicketsTable', 'substate'), get_seadb_column_name('TicketsTable', 'type')):
                case_insensitive = field_name == get_seadb_column_name('TicketsTable', 'state')
            ...
            elif field_name == get_seadb_column_name('TicketsTable', 'tags'):
                if isinstance(old_value, list):
                    old_value = [
                        get_option_id_by_name(metadata, get_seadb_column_name('TicketsTable', 'tags'), v)
                        for v in old_value
                    ]
    ...
    def get(self, request, project_uuid):
        ...
        try:
            ...
            ticket_column_name_to_return_name = {
                get_seadb_column_name('TicketsTable', 'substate'): 'substates',
                get_seadb_column_name('TicketsTable', 'type'): 'types',
                get_seadb_column_name('TicketsTable', 'state'): 'states'
            }
