...
class TestTicketsAPIView(TicketAPITestBase):
    def _view(self):
    ...
    def test_get_missing_view_id(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/', {})
        request.user = self.user
    ...
    def test_get_invalid_start_limit(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/', {'view_id': 'v1', 'start': '-1', 'limit': '-10'})
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=True):
            response = self._view()(request, project_uuid='p1')
        assert response.status_code == 400

    def test_get_non_int_start_limit_use_default(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/', {'view_id': 'v1', 'start': 'a', 'limit': 'b'})
        request.user = self.user
        project = Mock()
    ...
    def test_get_project_not_found(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/', {'view_id': 'v1', 'start': '0', 'limit': '10'})
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=True), \
    ...
    def test_get_permission_denied(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/', {'view_id': 'v1'})
        request.user = self.user
        project = Mock()
        project.workspace = Mock()
        project.workspace.owner = 'owner@auth.local'
    ...
    def test_post_feature_not_enabled(self):
        ...
        request = self.factory.post('/api/v2.1/projects/p1/tickets/', data=data)
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=False):
            resp = self._view()(request, project_uuid='p1')
        assert resp.status_code == 403

    def test_post_assignees_invalid_json(self):
        data = {
            'title': 't',
            'content': json.dumps({'text': 'c'}),
            'assignees': '{',
    ...
    def test_post_creation_interval_too_frequent(self):
        data = {
            'title': 't',
            'content': json.dumps({'text': 'c'}),
        }
    ...
    def test_put_missing_data(self):
        data = {}
        request = self.factory.put('/api/v2.1/projects/p1/tickets/', data=data, format='json')
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=True):
            resp = self._view()(request, project_uuid='p1')
        assert resp.status_code == 400

    def test_put_row_id_invalid(self):
    ...
    def test_put_invalid_content(self):
    ...
    def test_put_success_update_state_closed(self):
    ...
    # DELETE
    def test_delete_missing_ticket_ids(self):
        data = {}
        request = self.factory.delete('/api/v2.1/projects/p1/tickets/', data=data, format='json')
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=True):
            resp = self._view()(request, project_uuid='p1')
        assert resp.status_code == 400
...
class TestTicketAPIView(TicketAPITestBase):
    def _view(self):
    ...
    def test_get_ticket_not_found(self):
        ...
        with patch('seahub.tickets.tickets.is_org_context', return_value=True), \
            ...
            seadb_cls_mock.return_value = Mock()
            resp = self._view()(request, project_uuid='p1', ticket_id='1')
    ...
    def test_put_ticket_not_found(self):
        data = {'title': 't'}
        request = self.factory.put('/api/v2.1/projects/p1/tickets/1/', data=data, format='json')
        request.user = self.user
        project = Mock()
        project.workspace = Mock()
        project.workspace.owner = 'owner@auth.local'
    ...
    def test_put_priority_invalid(self):
        data = {'priority': 'abc'}
        request = self.factory.put('/api/v2.1/projects/p1/tickets/1/', data=data, format='json')
        request.user = self.user
        project = Mock()
    ...
    def test_put_success_state_closed_and_participants(self):
        ...
        assert self.user.username in update_row['participants']
        assert update_row['priority'] == 5

    def test_delete_ticket_not_found(self):
        request = self.factory.delete('/api/v2.1/projects/p1/tickets/1/', data={}, format='json')
        request.user = self.user
        project = Mock()
        project.workspace = Mock()
        project.workspace.owner = 'owner@auth.local'
...
class TestTicketsSearchAPIView(TicketAPITestBase):
    def _view(self):
        return TicketsSearchAPIView.as_view()

    def test_feature_not_enabled(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/search/', {'query': ''})
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=False):
            resp = self._view()(request, project_uuid='p1')
        assert resp.status_code == 403
    ...
    def test_get_success(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/search/', {'query': 'test'})
        request.user = self.user
        project = Mock()
        project.workspace = Mock()
        project.workspace.owner = 'owner@auth.local'
...
class TestMyTicketAPIView(TicketAPITestBase):
    def _view(self):
        return MyTicketAPIView.as_view()

    def test_feature_not_enabled(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/my/')
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=False):
            resp = self._view()(request, project_uuid='p1')
        assert resp.status_code == 403
    ...
    def test_get_invalid_view_id_default_open(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/my/', {'view_id': 'invalid'})
        request.user = self.user
        project = Mock()
        project.workspace = Mock()
        project.workspace.owner = 'owner@auth.local'
...
class TestTicketMetadataAPIView(TicketAPITestBase):
    def _view(self):
        return TicketMetadataAPIView.as_view()

    def test_feature_not_enabled(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/meta/')
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=False):
            resp = self._view()(request, project_uuid='p1')
        assert resp.status_code == 403
...
class TestTicketTrashAPIView(TicketAPITestBase):
    def _view(self):
        return TicketTrashAPIView.as_view()

    def test_feature_not_enabled(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/trash/')
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=False):
            resp = self._view()(request, project_uuid='p1')
        assert resp.status_code == 403
    ...
    def test_delete_no_deleted_tickets(self):
        request = self.factory.delete('/api/v2.1/projects/p1/tickets/trash/', data={}, format='json')
        request.user = self.user
        project = Mock()
        project.workspace = Mock()
        project.workspace.owner = 'owner@auth.local'
...
class TestTicketCommentsAPIView(TicketAPITestBase):
    def _view(self):
        return TicketCommentsAPIView.as_view()

    def test_get_feature_not_enabled(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/1/comments/')
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=False):
            resp = self._view()(request, project_uuid='p1', ticket_id='1')
        assert resp.status_code == 403
    ...
    def test_get_invalid_pagination_use_default(self):
        request = self.factory.get('/api/v2.1/projects/p1/tickets/1/comments/', {'page': 'a', 'per_page': 'b'})
        request.user = self.user
        project = Mock()
        project.workspace = Mock()
        project.workspace.owner = 'owner@auth.local'
    ...
    def test_post_creation_interval_too_frequent(self):
        ...
        request = self.factory.post('/api/v2.1/projects/p1/tickets/1/comments/', data=data)
        request.user = self.user
        project = Mock()
        project.workspace = Mock()
        project.workspace.owner = 'owner@auth.local'
...
class TestTicketCommentAPIView(TicketAPITestBase):
    def _view(self):
        return TicketCommentAPIView.as_view()

    def test_put_feature_not_enabled(self):
        request = self.factory.put('/api/v2.1/projects/p1/tickets/1/comments/1/', data={}, format='json')
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=False):
            resp = self._view()(request, project_uuid='p1', ticket_id='1', comment_id='1')
        assert resp.status_code == 403

    def test_put_missing_content(self):
        data = {}
        request = self.factory.put('/api/v2.1/projects/p1/tickets/1/comments/1/', data=data, format='json')
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=True):
            resp = self._view()(request, project_uuid='p1', ticket_id='1', comment_id='1')
        assert resp.status_code == 400
    ...
    def test_put_too_frequent(self):
        data = {'content': json.dumps({'text': 'hi'})}
        request = self.factory.put('/api/v2.1/projects/p1/tickets/1/comments/1/', data=data, format='json')
        request.user = self.user
        project = Mock()
    ...
    def test_delete_feature_not_enabled(self):
        request = self.factory.delete('/api/v2.1/projects/p1/tickets/1/comments/1/', data={}, format='json')
        request.user = self.user
        with patch('seahub.tickets.tickets.is_org_context', return_value=False):
            resp = self._view()(request, project_uuid='p1', ticket_id='1', comment_id='1')
        assert resp.status_code == 403
