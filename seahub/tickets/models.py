...
class TicketActivityManager(models.Manager):

    def record_activity(self, project_uuid, ticket_id, creator, activity_type, field_name='', old_value=None, new_value=None):
        project_uuid = uuid_str_to_32_chars(project_uuid)
        detail = {
    ...
    def record_activities(self, project_uuid, ticket_id, creator, changes):
        """ record activities in bulk
        changes: list of (activity_type, field_name, old_value, new_value)
        """
        project_uuid = uuid_str_to_32_chars(project_uuid)
        activities = []
    ...
    def get_activities(self, project_uuid, ticket_id, start=0, limit=50):
        project_uuid = uuid_str_to_32_chars(project_uuid)
        return list(self.filter(
            project_uuid=project_uuid,
            ticket_id=ticket_id
        ).order_by('created_time')[start:start+limit])
...
class TicketActivity(models.Model):
    ...
    detail = models.TextField(blank=True, default='{}')  # JSON field storing field_name, old_value, new_value
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['project_uuid', 'ticket_id', 'created_time'], name='idx_project_id_ticket_id_time'),
        ]
