...
from seahub.utils import render_error, is_org_context, check_slide_captcha_verified_time
...
@login_required
def subscription_trial_view(request):
    """
    subscription trial
    """
    if not subscription_check():
    ...
    if ENABLE_SLIDE_CAPTCHA and not check_slide_captcha_verified_time(request):
        error_msg = '滑动验证失败'
        return render_error(request, error_msg)
    ...
    try:
        ...
        url = SUBSCRIPTION_SERVER_URL.rstrip('/') + '/api/subscription/trial/'
        response = requests.post(url, json=data, headers=headers)
        response = handler_subscription_api_response(response)
        response_dic = response.json()
        if response.status_code >= 400:
        ...
        return render(request, 'subscription/pay_result.html', {'info': '试用版开通成功'})
    except Exception as e:
        logger.error(e)
        error_msg = _('Internal Server Error')
        return render_error(request, error_msg)
