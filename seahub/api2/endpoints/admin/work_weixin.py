...
def _import_user_from_work_weixin(api_user):
    uid = WORK_WEIXIN_UID_PREFIX + api_user.get('userid')
    try:
        contact_email = api_user.get('contact_email') if api_user.get('contact_email') else None  # replace '' with None
        ...
        logger.error('import user from work weixin api_user: %s, error: %s', api_user, e)
        return False
...
class AdminWorkWeixinDepartmentsImport(APIView):
    ...
    def _create_repo_workspace(self, request, group_id):
    ...
    def _create_group(self, request, group_name, username, parent_group_id):
        group_id = ccnet_api.create_group(group_name, username, parent_group_id=parent_group_id)
        return group_id
    ...
    def post(self, request):
        ...
        if not is_pro_version() or not admin_work_weixin_departments_check():
            error_msg = 'Feature is not enabled.'
            return api_error(status.HTTP_403_FORBIDDEN, error_msg)
        ...
        for index, department_obj in enumerate(api_department_list):
            ...
            if exist:
                ...
                try:
                    workspace = self._create_repo_workspace(request, exist_group.id)
                    logger.info('create workspace : %s for existed group: %s, group_id: %s', workspace.id,
                                new_group_name, exist_group.id)
                except Exception as e:
                    logger.error('existed group: %s, group_id: %s, create workspace error: %s', new_group_name,
                                 exist_group.id, e)
            ...
            try:
                ...
                group_id = self._create_group(request, new_group_name, DEPARTMENT_OWNER, parent_group_id)
                self._create_repo_workspace(request, group_id)
                ...
                logger.error(e)
                if group_id:  # if create group success but create workspace error, remove group
                    remove_group_common(group_id, DEPARTMENT_OWNER, -1)
                failed_msg = self._api_department_failed_msg(
