...
urlpatterns = [
    ## admin::sysinfo
    re_path(r'^sysinfo/$', SysInfo.as_view(), name='api-v1-sysinfo'),

    ## admin::users
    re_path(r'^users/$', AdminUsers.as_view(), name='api-v1-admin-users'),
    re_path(r'^search-user/$', AdminSearchUser.as_view(), name='api-v1-admin-search-user'),
    re_path(r'^search-user-by-org-id/$', AdminSearchUserByOrgId.as_view(), name='api-v1-admin-search-user-by-org-id'),

    ## admin::admin-role
    re_path(r'^admin-role/$', AdminAdminRole.as_view(), name='api-v1-admin-admin-role'),

    # [^...] Matches any single character not in brackets
    # + Matches between one and unlimited times, as many times as possible
    re_path(r'^users/(?P<email>[^/]+@[^/]+)/$', AdminUser.as_view(), name='api-v1-admin-user'),
    re_path(r'^users/(?P<email>[^/]+@[^/]+)/reset-password/$', AdminUserResetPassword.as_view(), name='api-v1-admin-user-reset-password'),
    re_path(r'^users/(?P<email>[^/]+@[^/]+)/groups/$', AdminUserGroups.as_view(), name='api-v1-admin-user-groups'),
    re_path(r'^users/(?P<email>[^/]+@[^/]+)/projects/$', AdminUserProjects.as_view(), name='api-v1-admin-user-projects'),
    re_path(r'^users/(?P<email>[^/]+@[^/]+)/two-factor-auth/$', TwoFactorAuthView.as_view(), name='api-v1-admin-user-two-factor-auth'),

    re_path(r'^admin-users/$', AdminAdminUsers.as_view(), name='api-v1-admin-admin-users'),

    ## admin::groups
    re_path(r'^groups/$', AdminGroups.as_view(), name='api-v1-admin-groups'),
    re_path(r'^search-group/$', AdminSearchGroup.as_view(), name='api-v1-admin-search-group'),
    re_path(r'^groups/(?P<group_id>\d+)/$', AdminGroup.as_view(), name='api-v1-admin-group'),
    ...
    re_path(r'^organizations/(?P<org_id>\d+)/groups/(?P<group_id>\d+)/$', AdminOrgGroup.as_view(), name='api-v1-admin-org-group'),
    re_path(r'^organizations/(?P<org_id>\d+)/projects/$', OrgProjects.as_view(), name='api-v1-admin-org-projects'),

    re_path(r'^search-organization/$', AdminSearchOrganization.as_view(), name='api-v1-admin-search-org'),
    re_path(r'^organizations-basic-info/$', AdminOrganizationsBaseInfo.as_view(), name='api-v1-admin-orgs-base-info'),
    ...
    ## admin::logo
    re_path(r'^license/$', AdminLicense.as_view(), name='api-v1-admin-license'),
