...
from seahub.api2.endpoints.admin.users import get_virtual_id_by_email, get_user_info
...
class AdminImportUsers(APIView):
    ...
    def post(self, request):
        ...
        try:
            fs = BytesIO(content)
            wb = load_workbook(filename=fs, read_only=True, data_only=True)
        except Exception as e:
            logger.error(e)

        # example file is like:
        # Contact email    Password ID(Optional) Name(Optional) Role(Optional) Space Quota(MB, Optional) Row Quota(Optional)
        # a@a.com          a        00001        a              default        1024                      5000
        # b@b.com          b        00002        b              default        2048                      5000

        records = []
        if wb.worksheets[0].max_column is not None and wb.worksheets[0].max_row is not None:
        ...
        for record in records:
            ...
            if record[2]:
                if IdInOrgTuple.objects.filter(org_id=-1, id_in_org=record[2]).exists():
                    result['failed'].append({
                        'email': email,
                        'error_msg': 'ID exists.'
                    })
            ...
            if record[2]:
                try:
                    IdInOrgTuple.objects.add_or_update(virtual_id, record[2], -1)
                except Exception as e:
                    logger.exception('set id_in_org for user: %s error: %s', virtual_id, e)
            ...
            try:
                if record[3]:
                    input_nickname = str(record[3]).strip()
            ...
            if record[5]:
                try:
                    space_quota_mb = int(record[5])
                    if space_quota_mb >= 0:
                        space_quota = int(space_quota_mb) * get_file_size_unit('MB')
                        UserQuota.objects.update_or_create(username=virtual_id, defaults={'asset_quota': space_quota})
                except Exception as e:
                    logger.error(e)
            ...
            if record[6]:
                try:
                    row_quota = int(record[6])
                    if row_quota >= 0:
                        UserQuota.objects.update_or_create(username=virtual_id, defaults={'row_limit': row_quota})
                except Exception as e:
                    logger.error(e)
