...
class OrgAdminAddressBookV2DepartmentsView(APIView):
    ...
    def get(self, request, org_id):
        ...
        if not parent_id or parent_id == '-1':
            ...
            parent_department = DepartmentsV2.objects.get_org_department_by_id(org_id, parent_id)
    ...
    def post(self, request, org_id):
        ...
        if parent_id != -1 and not name:
            return api_error(status.HTTP_400_BAD_REQUEST, 'name invalid')
        if parent_id == -1:
            name = request.user.org.org_name
        # resource check
        if parent_id != -1:
            parent_department = DepartmentsV2.objects.get_org_department_by_id(org_id, parent_id)
            if not parent_department:
        ...
        try:
            ...
            logger.error('org: %s create department error: %s', org_id, e)
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, 'Internal Server Error')

        return Response({'department': department.to_dict()})


class OrgAdminAddressBookV2DepartmentView(APIView):
    authentication_classes = (TokenAuthentication, SessionAuthentication)
    permission_classes = (IsAuthenticated, IsOrgAdmin)
    throttle_classes = (UserRateThrottle,)

    def put(self, request, org_id, department_id):
        org_id = int(org_id)
        if request.user.org.org_id != org_id:
            return api_error(status.HTTP_403_FORBIDDEN, 'Permission denied')
        name = request.data.get('name')
        # resource check
        department = DepartmentsV2.objects.get_org_department_by_id(org_id, department_id)
        if not department:
            return api_error(status.HTTP_404_NOT_FOUND, 'Department not found')
        if name:
        ...
        try:
            ...
            logger.error('org: %s update department error: %s', org_id, e)
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, 'Internal Server Error')
        return Response({'department': department.to_dict()})

    def delete(self, request, org_id, department_id):
        org_id = int(org_id)
        if request.user.org.org_id != org_id:
            return api_error(status.HTTP_403_FORBIDDEN, 'Permission denied')
        # resource check
        ...
        if not department:
            return api_error(status.HTTP_404_NOT_FOUND, 'Department not found')
        if DepartmentsV2.objects.has_sub_departments(department_id):
        ...
        try:
            ...
            logger.error('org: %s delete department error: %s', org_id, e)
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, 'Internal Server Error')

        return Response({'success': True})


class OrgAdminAddressBookV2DepartmentMembersView(APIView):
    authentication_classes = (TokenAuthentication, SessionAuthentication)
    permission_classes = (IsAuthenticated, IsOrgAdmin)
    throttle_classes = (UserRateThrottle,)

    def get(self, request, org_id, department_id):
        org_id = int(org_id)
        if request.user.org.org_id != org_id:
            return api_error(status.HTTP_403_FORBIDDEN, 'Permission denied')
        # resource check
    ...
    def post(self, request, org_id, department_id):
        org_id = int(org_id)
        if request.user.org.org_id != org_id:
            return api_error(status.HTTP_403_FORBIDDEN, 'Permission denied')
        # arguments check
...
class OrgAdminAddressBookV2DepartmentMemberView(APIView):
    authentication_classes = (TokenAuthentication, SessionAuthentication)
    permission_classes = (IsAuthenticated, IsOrgAdmin)
    throttle_classes = (UserRateThrottle,)

    def put(self, request, org_id, department_id, email):
        org_id = int(org_id)
        if request.user.org.org_id != org_id:
            return api_error(status.HTTP_403_FORBIDDEN, 'Permission denied')
        # arguments check
    ...
    def delete(self, request, org_id, department_id, email):
        org_id = int(org_id)
        if request.user.org.org_id != org_id:
            return api_error(status.HTTP_403_FORBIDDEN, 'Permission denied')
        # resource check
...
class OrgAdminAddUserToDepartmentsView(APIView):
    authentication_classes = (TokenAuthentication, SessionAuthentication)
    permission_classes = (IsAuthenticated, IsOrgAdmin)
    throttle_classes = (UserRateThrottle,)
    ...
    def post(self, request, org_id):
        ...
        for org in orgs:
            if org.org_id == org_id:
                ...
                break
        if not flag:
            return api_error(status.HTTP_400_BAD_REQUEST, 'User is not a member of the orgnization')
        if request.user.org.org_id != org_id:
            return api_error(status.HTTP_403_FORBIDDEN, 'Permission denied')
        ...
        departments = list(DepartmentsV2.objects.filter(org_id=org_id, id__in=department_ids))
        if not departments:
            return api_error(status.HTTP_400_BAD_REQUEST, 'department_ids invalid')
...
class OrgAdminDepartmentsMigrateView(APIView):
    authentication_classes = (TokenAuthentication, SessionAuthentication)
    permission_classes = (IsAuthenticated, IsOrgAdmin)
    throttle_classes = (UserRateThrottle,)

    def migrate_department_v1(self, department_v1, parent_id, org_id, group_ids):
        ...
        department_v2 = DepartmentsV2.objects.create_department(name_v2, parent_id, org_id)
        ...
        for sub_department_v1 in sub_deparmtents_v1:
            self.migrate_department_v1(sub_department_v1, department_v2.id, org_id, group_ids)
        # set department_v1 to common group
        ## update parent_group_id to 0 on ccnet db
        update_group_info(department_v1.id, {'parent_group_id': 0})
    ...
    def post(self, request, org_id):
        ...
        if request.user.org.org_id != org_id:
            return api_error(status.HTTP_403_FORBIDDEN, 'Permission denied')
        top_department_v2 = DepartmentsV2.objects.get_org_top_departments(org_id).first()
        ...
        departments_v1 = ccnet_api.get_org_top_groups(org_id)
        departments_v2 = []
        group_ids = []
        try:
            with transaction.atomic():
                for department_v1 in departments_v1:
                    department_v2 = self.migrate_department_v1(department_v1, top_department_v2.id, org_id, group_ids)
...
class OrgAdminAddressBookV2DepartmentGroupView(APIView):
    authentication_classes = (TokenAuthentication, SessionAuthentication)
    permission_classes = (IsAuthenticated, IsOrgAdmin)
    throttle_classes = (UserRateThrottle,)
...
class OrgAdminNonAddressBookV2UsersView(APIView):
    authentication_classes = (TokenAuthentication, SessionAuthentication)
    permission_classes = (IsAuthenticated, IsOrgAdmin)
    throttle_classes = (UserRateThrottle,)
    ...
    def get(self, request, org_id):
        ...
        sql = '''
            SELECT ou.email FROM %(ccnet_db)s.OrgUser ou
            LEFT JOIN %(ccnet_db)s.EmailUser eu ON ou.email=eu.email
            LEFT JOIN department_members_v2 dm ON ou.email=dm.username
            WHERE ou.org_id=%(org_id)s AND eu.is_active=1 AND dm.id IS NULL
        '''
        emails = []
        try:
