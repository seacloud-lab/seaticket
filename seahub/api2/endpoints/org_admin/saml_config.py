...
class OrgSAMLConfigView(APIView):
    ...
    def put(self, request, org_id):
        ...
        if not metadata_url and not domain and not idp_certificate:
            return api_error(status.HTTP_400_BAD_REQUEST, 'metadata_url or domain or idp_certificate invalid.')
        ...
        if domain:
            exist_config = OrgSAMLConfig.objects.get_config_by_domain(domain)
            if exist_config:
                error_msg = 'Domain %s already exists' % domain
                return api_error(status.HTTP_400_BAD_REQUEST, error_msg)
            saml_config.domain = domain
            saml_config.dns_txt = 'seatable-site-verification=' + \
                str(uuid.uuid4().hex)
...
class OrgVerifyDomain(APIView):
    ...
    def put(self, request, org_id):
        ...
        if saml_config.domain != domain:
            error_msg = 'Domain %s not belong organization %s.' % (
                domain, org.org_name)
            return api_error(status.HTTP_404_NOT_FOUND, error_msg)
