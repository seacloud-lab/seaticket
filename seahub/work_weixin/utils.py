...
def work_weixin_base_check():
    ...
    if not WORK_WEIXIN_CORP_ID or not WORK_WEIXIN_AGENT_SECRET or not WORK_WEIXIN_ACCESS_TOKEN_URL:
        logger.warning('work weixin base relevant settings invalid.')
        logger.warning('please check WORK_WEIXIN_CORP_ID, WORK_WEIXIN_AGENT_SECRET')
        logger.warning('WORK_WEIXIN_ACCESS_TOKEN_URL: %s' % WORK_WEIXIN_ACCESS_TOKEN_URL)
        return False
...
def work_weixin_oauth_check():
    ...
    if not WORK_WEIXIN_AGENT_ID \
        ...
        logger.warning('work weixin oauth relevant settings invalid.')
        logger.warning('please check WORK_WEIXIN_AGENT_ID')
        logger.warning('WORK_WEIXIN_GET_USER_INFO_URL: %s' % WORK_WEIXIN_GET_USER_INFO_URL)
        logger.warning('WORK_WEIXIN_AUTHORIZATION_URL: %s' % WORK_WEIXIN_AUTHORIZATION_URL)
        logger.warning('WORK_WEIXIN_GET_USER_PROFILE_URL: %s' % WORK_WEIXIN_GET_USER_PROFILE_URL)
        return False
...
def admin_work_weixin_departments_check():
    ...
    if not WORK_WEIXIN_DEPARTMENTS_URL \
        ...
        logger.warning('admin work weixin departments relevant settings invalid.')
        logger.warning('WORK_WEIXIN_DEPARTMENTS_URL: %s' % WORK_WEIXIN_DEPARTMENTS_URL)
        logger.warning('WORK_WEIXIN_DEPARTMENT_MEMBERS_URL: %s' % WORK_WEIXIN_DEPARTMENT_MEMBERS_URL)
        return False
...
def work_weixin_notifications_check():
    ...
    if not WORK_WEIXIN_AGENT_ID \
        ...
        logger.warning('work weixin notifications relevant settings invalid.')
        logger.warning('please check WORK_WEIXIN_AGENT_ID')
        logger.warning('WORK_WEIXIN_NOTIFICATIONS_URL: %s' % WORK_WEIXIN_NOTIFICATIONS_URL)
        return False
...
def update_work_weixin_user_info(api_user):
    ...
    nickname = api_user.get('name') or '新用户-' + api_user.get('UserId', '')[-2:]
