...
from seahub.settings import SEAQA_EVENTS_INNER_SERVER_URL
...
def convert_kb_view_to_excel(params):
    url = urljoin(SEAQA_EVENTS_INNER_SERVER_URL, '/convert-kb-view-to-excel')
...
def query_kb_task_status(task_id):
    url = urljoin(SEAQA_EVENTS_INNER_SERVER_URL, '/kb-task-status')
    headers = {"Authorization": "Token %s" % _sign_auth_token()}
