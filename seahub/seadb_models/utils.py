...
def ensure_skills_seadb_table(seadb_api, project_uuid):
    metadata = seadb_api.get_base_metadata(project_uuid)
    tables_metadata = metadata.get('tables') or []

    skills_table_name = SchemaTables.SKILLS.table_name()
    skills_table = get_current_table_metadata(tables_metadata, skills_table_name)
    if not skills_table:
        init_seadb_tables_from_schema([SchemaTables.SKILLS], seadb_api, project_uuid)
