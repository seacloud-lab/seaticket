...
class GitHubSeaDBAPI:
    ...
    def _get_first_issue_comment_by_id(self, connection_id, issue_id):
        table_name = SchemaTables.GITHUB_ISSUE_COMMENTS.table_name(connection_id)
        first_comment_sql = "SELECT `comment_id`, `issue_id`, `author`, `content`, `created_time` " \
            f"FROM `{table_name}` WHERE `issue_id` = {issue_id} ORDER BY `comment_id` ASC LIMIT 1"
        first_comments = self.seadb_api.query_rows(self.base_id, first_comment_sql).get('results', [])
        return first_comments[0] if first_comments else None

    def _get_latest_issue_comments_by_id(self, connection_id, issue_id, limit_for_each_id):
        table_name = SchemaTables.GITHUB_ISSUE_COMMENTS.table_name(connection_id)
        latest_comments_sql = "SELECT `comment_id`, `issue_id`, `author`, `content`, `created_time` " \
            f"FROM `{table_name}` WHERE `issue_id` = {issue_id} ORDER BY `comment_id` DESC LIMIT {limit_for_each_id}"
        return self.seadb_api.query_rows(self.base_id, latest_comments_sql).get('results', [])

    def _get_selected_issue_comments_for_attachment(self, connection_id, issue_id, body_content_length, limit_for_each_id):
        if body_content_length >= ATTACHMENT_CONTENT_MAX_SIZE:
            return [_build_comment_notice(_COMMENTS_OMITTED_NOTICE)]

        first_comment = self._get_first_issue_comment_by_id(connection_id, issue_id)
        ...
        latest_comments = self._get_latest_issue_comments_by_id(connection_id, issue_id, limit_for_each_id)
        comments_by_id = {
        ...
        for connection_id, _pks in connection_ids_pks_map.items():
            ...
            for issue_data in issues:
                ...
                comments = self._get_selected_issue_comments_for_attachment(
                    connection_id, issue_data['issue_id'], len(full_content), ATTACHMENT_ISSUE_MAX_COMMENTS,
                )
                for comment in comments:
