...
from seahub.avatar.settings import AVATAR_DEFAULT_URL, AVATAR_CACHE_TIMEOUT,\
    ...
    AVATAR_FILE_STORAGE, APP_AVATAR_DEFAULT_URL
...
def get_app_default_avatar_url():
    base_url = getattr(settings, 'MEDIA_URL', '')
    # Don't use base_url if the default avatar url starts with http:// of https://
    if APP_AVATAR_DEFAULT_URL.startswith('http://') or APP_AVATAR_DEFAULT_URL.startswith('https://'):
        return AVATAR_DEFAULT_URL
    # We'll be nice and make sure there are no duplicated forward slashes
    ends = base_url.endswith('/')
    begins = APP_AVATAR_DEFAULT_URL.startswith('/')
    if ends and begins:
        base_url = base_url[:-1]
    elif not ends and not begins:
        return '%s/%s' % (base_url, APP_AVATAR_DEFAULT_URL)
    return '%s%s' % (base_url, APP_AVATAR_DEFAULT_URL)
...
def get_avatar_file_storage():
    ...
    # database storage 
    elif AVATAR_FILE_STORAGE == 'seahub.base.database_storage.DatabaseStorage':
        dbs_options = {
    ...
    # oss storage
    elif AVATAR_FILE_STORAGE == 'django_oss_storage.backends.OssStorage':
        storage = get_storage_class(AVATAR_FILE_STORAGE)()
        storage.location = ''
        return storage
    # S3 storage
    elif AVATAR_FILE_STORAGE == 'django_s3_storage.storage.S3Storage':
        s3_kwargs = {
            'AWS_S3_MAX_AGE_SECONDS'.lower(): 60 * 60 * 24,
            'AWS_S3_BUCKET_AUTH'.lower(): False,
        }
        ...
        storage = get_storage_class(AVATAR_FILE_STORAGE)(**s3_kwargs)
        return storage
    #
    else:
        return get_storage_class(AVATAR_FILE_STORAGE)()
