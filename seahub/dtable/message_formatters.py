...
def fill_message_with_sql_row(msg, columns, row, empty_image_files=True, convert_to_html=False):
    if not msg:
        return ''
    ...
    for blank in blanks:
        ...
        if not formatter_class:
            continue
        params = create_formatter_params(column_type, value=value, convert_to_html=convert_to_html)
        if value is None:
...
def fill_message_by_dtable_db(username, dtable_uuid, table_name, row_id, msg):
    ...
    try:
        ...
        if not error_message:
            ...
            if rows:
                msg = fill_message_with_sql_row(msg, columns, rows[0])
            else:
