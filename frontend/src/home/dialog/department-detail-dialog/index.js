...
import DepartmentGroupMemberSelected from './department-group-member-selected';
import { enableAddressBookV2, gettext, isOrgContext, username, mediaUrl } from '../../../constants';

import '../manage-members-dialog/index.css';
import './index.css';
...
class DepartmentDetailDialog extends React.Component {
  ...
  onMemberChecked = (member) => {
    ...
    if (searchVal || this.state.departmentMembers.indexOf(member) !== -1) {
  ...
  render() {
    let { departmentsLoading, departments, searchVal, searchResults } = this.state;
