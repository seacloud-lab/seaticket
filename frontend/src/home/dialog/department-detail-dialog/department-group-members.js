...
class DepartmentGroupMembers extends Component {
  ...
  getDisplayMembers = () => {
    const { searchVal, searchResults, members } = this.props;
    const displayMembers = searchVal.length > 0 ? searchResults : members;
    return displayMembers;
  };

  selectAll = () => {
    const displayMembers = this.getDisplayMembers();
    this.props.selectAll(displayMembers);
  };
  ...
  getEnableSelectAll = () => {
    const { memberSelected, selectedMemberMap } = this.props;
    const displayMembers = this.getDisplayMembers();
    if (!memberSelected || !Array.isArray(displayMembers)) {
      return false;
    }
    const selectedMap = Object.assign({}, memberSelected, selectedMemberMap);
    for (let i = 0; i < displayMembers.length; i++) {
      const email = displayMembers[i].email;
  ...
  render() {
    const { memberSelected, loading, selectedMemberMap, currentDepartment, usedFor } = this.props;
    const displayMembers = this.getDisplayMembers();
    let headerTitle;
      ...
      <div className="department-dialog-member pt-4">
        <div className="w-100">
          ...
          {displayMembers.length > 0 ?
            <Fragment>
              <table className="department-dialog-member-table" onScroll={this.handleScroll}>
                <tbody>
