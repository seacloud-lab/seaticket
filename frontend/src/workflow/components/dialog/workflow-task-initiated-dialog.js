...
import { getValidWorkflowColumns, getDisplayNodeColumns } from '../../utils/utils';
...
class WorkflowTaskInitiatedDialog extends Component {
  ...
  componentDidMount() {
    ...
    dtableWebAPI.getWorkflowTaskInitiatorView(this.token, this.taskId).then(res => {
      ...
      this.userService.getRelatedUsers(this.workspaceId, this.dtableName, { workflowToken: this.token }, this.updateCollaborators);
      this.workflowColumns = getValidWorkflowColumns(this.table.columns, columns_config, tables, table._id);
      this.displayColumns = getDisplayNodeColumns(this.workflowColumns, show_columns);
...
WorkflowTaskInitiatedDialog.propTypes = {
  workflowTask: PropTypes.object.isRequired,
  onToggle: PropTypes.func.isRequired,
};

export default WorkflowTaskInitiatedDialog;
