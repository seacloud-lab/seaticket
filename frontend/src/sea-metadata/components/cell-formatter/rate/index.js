...
const RateFormatter = ({ value, data, className, children: emptyFormatter }) => {
  ...
  for (let i = 0; i < validValue; i++) {
    rateList.push(
      <div className="sea-metadata-rate-item" style={{ fill: color || '#eee' }} key={i}>
        <Icon symbol={type} />
      </div>
    );
  }
  ...
  return (
    <div className={classnames('sea-metadata-ui cell-formatter-container rate-formatter', className)}>
      {rateList}
    </div>
  );
};
