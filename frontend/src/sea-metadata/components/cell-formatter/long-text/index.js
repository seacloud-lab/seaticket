...
const LongTextFormatter = ({ value: oldValue, className, previewClassName, rowHeightClassName, children: emptyFormatter, }) => {
  ...
  const renderLinks = useCallback(() => {
    ...
    return (
      <span className="sea-metadata-long-text-links">
        <Icon symbol="url" />
        {links.length}
      </span>
    );
  ...
  const renderCheckList = useCallback(() => {
    ...
    return (
      <span className={classnames('sea-metadata-long-text-check-list', { 'completed': checkList.completed === checkList.total })}>
  ...
  const renderImages = useCallback(() => {
    ...
    return (
      <span className="sea-metadata-long-text-images">
        <img src={images[0]} alt=""/>
        <i className="sea-metadata-image-number">{images.length > 1 ? '+' + images.length : null}</i>
      </span>
    );
  }, [value]);
  ...
  return (
    <div
      className={classnames('sea-metadata-ui cell-formatter-container long-text-formatter', className)}
