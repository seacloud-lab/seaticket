...
const MultipleSelectEditor = forwardRef(({
  ...
  const renderOptions = useCallback(() => {
    ...
    return displayOptions.map((option, i) => {
      ...
      return (
        <div key={option.id} className="sea-metadata-single-select-item" ref={selectItemRef}>
          <div
  ...
  return (
    <div className="sea-metadata-single-select-editor sea-metadata-multiple-select-editor" style={style} ref={editorRef}>
      <DeleteOption value={value} options={options} onDelete={onDeleteOption} />
      <div className="sea-metadata-search-single-select-options">
        <SearchInput
          placeholder={gettext('Search option')}
          onKeyDown={onKeyDown}
          onChange={onChangeSearch}
          autoFocus={true}
          className="sea-metadata-search-options"
        />
      </div>
      <div className="sea-metadata-single-select-editor-container" ref={editorContainerRef}>
        {renderOptions()}
      </div>
      {isShowCreateBtn && (
        <CommonAddTool
