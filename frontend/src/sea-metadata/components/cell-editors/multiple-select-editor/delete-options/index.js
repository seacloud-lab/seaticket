...
const DeleteOption = ({ value, options, onDelete }) => {
  ...
  return (
    <div className="sea-metadata-delete-select-options">
      {displayOptions.map(option => {
        ...
        return (
          <div key={id} className="sea-metadata-delete-select-option" style={style}>
            <span className="sea-metadata-delete-select-option-name text-truncate" title={name} aria-label={name}>{name}</span>
            <IconBtn className="sea-metadata-delete-select-remove" onClick={(event) => onDelete(id, event)} symbol="x-01" />
          </div>
        );
      })}
    </div>
