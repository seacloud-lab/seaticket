...
const RadioGroup = ({ value, options, className, onChange: onChangeAPI }) => {
  ...
  return (
    <div className={classnames('sea-metadata-radio-group', className)} data-active={value}>
      {options.map(option => {
        ...
        return (
          <div
            key={value}
            data-option={value}
            className={classnames('sea-metadata-radio-group-option', { 'active': value === selected })}
            onClick={onChange}
          >
            {label}
          </div>
