...
const Formatter = ({ column, value, isSample, queryUserAPI, emptyTip, openExpandedRow, row, ...params }) => {
  ...
  switch (columnType) {
    case CellType.TEXT: {
      return (
        <TextFormatter value={value} column={column} className={className} openExpandedRow={() => openExpandedRow && openExpandedRow(row)} >
    ...
    case CellType.TAGS: {
      return (
        <TagsFormatter value={value} className={className} showName={true}>
          <Empty columnType={columnType} placeholder={emptyTip} />
        </TagsFormatter>
      );
    }
