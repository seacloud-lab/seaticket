...
const HeaderDropdownMenu = forwardRef(({
  ...
  const renderDateFormat = useCallback((canModifyColumnData) => {
    ...
    return (
      <Dropdown className="sea-metadata-dropdown-menu w-100" isOpen={isSubMenuShow} direction="right">
        <DropdownToggle
          tag="span"
          role="button"
          data-toggle="dropdown"
          aria-expanded={isMenuShow}
          className="dropdown-item sea-metadata-column-dropdown-item d-flex align-items-center"
  ...
  const renderDropdownMenu = useCallback(() => {
    ...
    return (
      <DropdownMenu ref={menuRef} className="sea-qa-dropdown-menu sea-metadata-column-dropdown-menu">
        <div ref={dropdownDomRef}>
          {/* {type === CellType.SINGLE_SELECT && (
            <>
              <DropdownItem
                disabled={!canModifyColumnData}
                target="sea-metadata-edit-column-options"
          ...
          <DropdownItem
            ...
            target="sea-metadata-rename-column"
            iconName="rename"
            title={gettext('Rename')}
            tip={isPrivateColumn ? gettext('This is not editable') : gettext('You do not have permission')}
  ...
  return (
    <>
      <Dropdown className="sea-metadata-dropdown-menu" isOpen={isMenuShow} toggle={onToggle}>
        <DropdownToggle
          tag="span"
          role="button"
          data-toggle="dropdown"
        ...
        {isMenuShow && !isMobile &&
          <ModalPortal>
            <div className="sea-metadata-dropdown-menu large">{renderDropdownMenu()}</div>
          </ModalPortal>
        }
      </Dropdown>
      {isRenamePopoverShow && (
        <ModalPortal>
          <RenamePopover
            target={`sea-metadata-column-${column.key}`}
            value={column.name}
