 ...
 */
const sortRowsWithMultiSorts = (tableRows, sorts, { collaborators, tagsData }) => {
  tableRows.sort((currentRow, nextRow) => {
    let initValue = 0;
    sorts.forEach((sort) => {
      const { sort_type, column } = sort;
      const { type: columnType } = column;
      let currCellVal = getCellValueByColumn(currentRow, column, { tagsData });
      let nextCellVal = getCellValueByColumn(nextRow, column, { tagsData });
 ...
 */
const sortTableRows = (table, rows, sorts, { collaborators, tagsData, typesData, isReturnID = true } = {}) => {
  const { columns } = table;
  if (!Array.isArray(rows) || rows.length === 0) return [];
  const sortRows = rows.slice(0);
  const validSorts = deleteInvalidSort(sorts, columns, typesData);
  sortRowsWithMultiSorts(sortRows, validSorts, { collaborators, tagsData });
