...
function convert2Text(cellValue, oldCellValue, fromColumn, { collaborators, tagsData } = {}) {
  const { type: fromColumnType, data: fromColumnData } = fromColumn;
  switch (fromColumnType) {
    case CellType.TEXT: {
...
function convert2Collaborator(cellValue, oldCellValue, fromColumnType, { collaborators = [] } = {}) {
  switch (fromColumnType) {
    case CellType.COLLABORATOR: {
...
const convert2MultipleSelect = (copiedCellVal, pasteCellVal, copiedColumn, pasteColumn, api) => {
  ...
  if (newCellOptions && newCellOptions.length > 0) {
    if (!context.canModifyColumnData(pasteColumn)) return null;
...
function convertCellValue(cellValue, oldCellValue, targetColumn, fromColumn, { collaborators, tagsData, api }) {
  ...
  switch (targetColumnType) {
    ...
    case CellType.TEXT: {
      return convert2Text(cellValue, oldCellValue, fromColumn, { collaborators, tagsData });
    }
    case CellType.COLLABORATOR: {
      return convert2Collaborator(cellValue, oldCellValue, fromColumnType, { collaborators });
    }
    case CellType.RATE: {
