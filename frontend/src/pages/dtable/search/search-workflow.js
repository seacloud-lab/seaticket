...
class SearchWorkflow extends Component {
  ...
  onEnter = () => {
    if (this.state.resultItems.length > 0 && this.workflowRefs[this.state.highLightIndex]) {
      this.workflowRefs[this.state.highLightIndex].toggleWorkflowTasks();
    }
  };
  ...
  sendRequest = (queryData) => {
    ...
    dtableWebAPI.searchItems(query_str, 'workflow').then(res => {
      let workflows = res.data.results;
      this.setState({
        highLightIndex: 0,
        resultItems: workflows,
        isResultGetted: true,
      });
    });
  };
  ...
  refreshPendingtasksCount = () => {
    dtableWebAPI.getWorkflowOngoingTasksCount().then(res => {
      ...
      this.eventBus.dispatch(EVENT_OPERATION_TYPE.UPDATE_WORKFLOW_TASK_COUNT, newCount);
    }).catch(error => {
    ...
    return (
      <ul className="search-result-list" ref={ref => this.ulRef = ref}>
        {resultItems.map((item, index) => {
          const className = `search-workflow-item ${highLightIndex === index ? 'workflow-item-selected' : ''}`;
          return (
            <WorkflowItem
              key={item.id + index}
              index={index}
              ref={ref => this.workflowRefs[index] = ref}
              workflowItem={item}
              className={className}
    ...
    return (
      <Fragment>
        ...
        <MediaQuery query="(max-width: 767.8px)">
          <div className="search-icon-container workflow-page">
...
SearchWorkflow.propTypes = propTypes;
