...
class TaskItem extends React.Component {
  ...
  render() {
    ...
    let itemLink = workflowTaskItem.transfer_url;
    return (
...
class MainPanelWorkflowOngoingTasks extends Component {
  ...
  loadData = (page) => {
    ...
    dtableWebAPI.listWorkflowOngoingTasks(page, perPage).then(res => {
      this.setState({
        isLoading: false,
  ...
  render() {
    ...
    if (taskList.length === 0) return (
      <NoWorkflow
        ...
        description={gettext('Workflow tasks need you to handle will be shown here.')}
      />
    );
      ...
      <div className="main-panel-center">
        <div className="cur-view-container" id="forms">
          <div className="cur-view-content d-block" onScroll={this.handleScroll}>
            <div className="workflow-title">{gettext('My pending tasks')}</div>
            {this.state.errorMsg &&
                ...
                <table className="table-hover activity-table">
                  <thead>
                    <tr>
                      ...
                      <th width={isDesktop ? '60%' : '60%'}>{gettext('Workflow')}</th>
                      <th width={isDesktop ? '35%' : '30%'}>{gettext('Creator')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {taskList.map(task => {
...
export default MainPanelWorkflowOngoingTasks;
