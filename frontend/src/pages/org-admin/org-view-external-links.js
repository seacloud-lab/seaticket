...
import { orgID, gettext, mediaUrl } from '../../utils/constants';
...
class Item extends React.Component {
  ...
  deleteExternalLink = () => {
    ...
    orgAdminServiceApi.orgAdminDeleteViewExternalLink(orgID, linkToken).then(() => {
  ...
  visitExternalLink = () => {
    const view_url = this.props.item.url;
...
const OrgViewExternalLinksPropTypes = {

};

class OrgViewExternalLinks extends React.Component {
  constructor(props) {
    super(props);
    this.state = ({
      isItemFreezed: false,
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      this.loadViewExternalLinks(this.state.page);
    });
  }

  loadViewExternalLinks(page) {
    orgAdminServiceApi.orgAdminListViewExternalLinks(orgID, page, this.state.perPage).then((res) => {
      this.setState({
  ...
  render() {
    ...
    const emptyTip = (
      <DTableEmptyTip src={`${mediaUrl}img/no-items-tip.png`} text={gettext('No view external links')} />
    );
    const linkList = (
...
OrgViewExternalLinks.propTypes = OrgViewExternalLinksPropTypes;

export default OrgViewExternalLinks;
