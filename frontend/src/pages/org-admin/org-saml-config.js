...
class OrgSAMLConfig extends Component {
  ...
  componentDidMount() {
    orgAdminServiceApi.orgAdminGetSamlConfig(orgID).then((res) => {
      this.setState({
        loading: false,
  ...
  updateSamlConfig = (changeType, value) => {
    ...
    orgAdminServiceApi.orgAdminUpdateSamlConfig(orgID, metadataUrl, domain, idpCertificate).then((res) => {
      this.setState({
        samlConfigID: res.data.saml_config.id,
        metadataUrl: res.data.saml_config.metadata_url,
        domain: res.data.saml_config.domain || '',
      ...
      toaster.success(gettext('SAML updated'));
    }).catch((error) => {
  ...
  verifyDomain = () => {
    ...
    orgAdminServiceApi.orgAdminVerifyDomain(orgID, domain).then((res) => {
  ...
  render() {
    ...
    let entityID = `${serviceURL}/org/custom/${orgID}/saml/metadata/`;
    let acsURL = `${serviceURL}/org/custom/${orgID}/saml/acs/`;
    let logoutURL = `${serviceURL}/org/custom/${orgID}/saml/ls/`;
            ...
            <div className="cur-view-content container mw-100 px-4">
              ...
              {(!loading && !errorMsg) &&
                <Fragment>
                  ...
                  <Section headingText={gettext('Configure SeaTable')}>
                    <p className="text-secondary mt-1">{gettext('Use information from your Identity Provider to configure SeaTable')}</p>
                    <Fragment>
                      <InputItem
                        value={metadataUrl}
...
OrgSAMLConfig.propTypes = propTypes;
