...
class VirusFileItem extends Component {
  ...
  getGroupOrUser = () => {
    const { virusFile } = this.props;
    let groupOrUserLink;
    if (virusFile.group_id) {
      const groupUrl = `${siteRoot}sys/groups/${virusFile.group_id}/dtables/`;
      groupOrUserLink = <Link to={groupUrl}>{virusFile.group_name}</Link>;
    } else if (virusFile.username) {
      const userUrl = `${siteRoot}sys/users/${encodeURIComponent(virusFile.username)}/`;
      groupOrUserLink = <Link to={userUrl}>{virusFile.nickname}</Link>;
    } else {
      groupOrUserLink = null;
    }
    let orgModule;
    if (virusFile.org_id !== -1) {
      const orgUrl = `${siteRoot}sys/organizations/${virusFile.org_id}/info/`;
      orgModule = (
        <Fragment>
          <br />
          <Link to={orgUrl}>({virusFile.org_name})</Link>
        </Fragment>
      );
    } else {
      orgModule = null;
  ...
  render() {
    ...
    return (
      <tr className={this.state.highlight ? 'tr-highlight' : ''} onMouseEnter={this.handleMouseEnter} onMouseLeave={this.handleMouseLeave}>
        ...
        <td>{this.getGroupOrUser()}</td>
        <td>{virusFile.dtable_name}</td>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th width="24%">{gettext('Group')} / {gettext('User')}</th>
...
class UnhandledVirusFiles extends Component {
  ...
  getListByPage = (page) => {
    ...
    sysAdminServiceApi.listVirusFiles(page, perPage, hasHandled).then((res) => {
      const data = res.data;
  ...
  handleFile = (virusID, op) => {
    ...
    switch (op) {
      case 'delete':
        request = sysAdminServiceApi.deleteVirusFile(virusID);
        break;
      case 'ignore':
        request = sysAdminServiceApi.toggleIgnoreVirusFile(virusID, true);
        break;
      case 'do-not-ignore':
        request = sysAdminServiceApi.toggleIgnoreVirusFile(virusID, false);
        break;
      default:
        break;
    }
  ...
  handleSelectedItems = (op) => {
    ...
    sysAdminServiceApi.batchProcessVirusFiles(virusIDs, op).then((res) => {
