...
const contentPropTypes = {
  ...
  adminListNotificationRules: PropTypes.func.isRequired,
...
class Content extends Component {
  ...
  getPreviousPageList = () => {
    this.props.adminListNotificationRules(this.props.currentPage - 1);
  };
      ...
      const table = (
        <Fragment>
          <table>
            <thead>
              <tr>
                <th width="11%">{gettext('Run condition')}</th>
                <th width="11%">{gettext('Rule name')}</th>
                <th width='11%'>{gettext('Condition')}</th>
                <th width="11%">{gettext('Type')}</th>
                <th width="12%">{gettext('Base ID')}</th>
                <th width="11%">{gettext('Creator')}</th>
                <th width="11%">{gettext('Created at')}</th>
                <th width="11%">{gettext('Last trigger time')}</th>
                <th width="11%">{gettext(/* Operations*/)}</th>
              </tr>
            </thead>
            <tbody>
...
class NotificationRules extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      this.adminListNotificationRules(this.state.currentPage);
    });
  }
  ...
  resetPerPage = (perPage) => {
    this.setState({
      ...
      this.adminListNotificationRules(1);
    });
  };

  adminListNotificationRules = (page) => {
    let { perPage } = this.state;
    sysAdminServiceApi.sysAdminListAllNotificationRules(page, perPage).then((res) => {
      this.setState({
        loading: false,
        notificationRuleList: res.data.notification_rule_list,
  ...
  adminDeleteNotificationRule = (id) => {
    sysAdminServiceApi.sysAdminDeleteNotificationRule(id).then(res => {
      let notificationRuleList = this.state.notificationRuleList.filter(item => {
        return item.id !== id;
      });
      this.setState({ notificationRuleList: notificationRuleList });
      ...
      <Fragment>
        ...
        <div className="main-panel-center flex-row">
          <div className="cur-view-container">
            <DTableRulesNav currentItem='notification-rules' />
            <div className="cur-view-content">
              <Content
                loading={this.state.loading}
                errorMsg={this.state.errorMsg}
                items={this.state.notificationRuleList}
                adminDeleteNotificationRule={this.adminDeleteNotificationRule}
                adminListNotificationRules={this.adminListNotificationRules}
...
NotificationRules.propTypes = notificationRulePropTypes;
