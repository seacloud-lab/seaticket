...
import { loginUrl, gettext, siteRoot, mediaUrl } from '../../../constants';
...
class OrgExternalLinks extends Component {
  ...
  getExternalLinkListByPage = (page) => {
    sysAdminServiceApi.sysAdminListExternalLinks(page, this.state.perPage, { orgID: this.props.orgID }).then((res) => {
      this.setState({
        loading: false,
  ...
  deleteExternalLink = (token) => {
    sysAdminServiceApi.sysAdminDeleteExternalLink(token).then(res => {
