...
class WorkWeixinDepartments extends Component {
  ...
  getWorkWeixinDepartmentsList = (departmentID) => {
    sysAdminServiceApi.adminListWorkWeixinDepartments(departmentID).then((res) => {
      if (!departmentID) {
      ...
      if (error.response && error.response.status === 403) {
        window.location = siteRoot + 'sys/info/';
      }
    });
  };
  ...
  getWorkWeixinDepartmentMembersList = (department_id) => {
    ...
    sysAdminServiceApi.adminListWorkWeixinDepartmentMembers(department_id, { fetch_child: true }).then((res) => {
  ...
  onSubmit = () => {
    ...
    sysAdminServiceApi.adminAddWorkWeixinUsersBatch(userList).then((res) => {
      this.setState({
        newUsersTempObj: {},
        isCheckedAll: false,
      });
  ...
  onImportDepartmentSubmit = () => {
    ...
    sysAdminServiceApi.adminImportWorkWeixinDepartment(importDepartment.id).then((res) => {
      this.setState({
        isMembersListLoading: true,
