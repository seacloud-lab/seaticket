...
import { getNormalizedPageContent, isExitViewTableWidget, getLinkWidgets } from './page-design/utils/widget-utils';
import DTableAPIGateway from './api/dtable-api-gateway';

const { dtableMetadata, page_id, row_id, collaborators, dtable_uuid, access_token, dtableWebURL } = window.app.pageOptions;
...
class DtableRowPageDesign extends Component {
  ...
  componentDidMount() {
    ...
    fetch(content_url).then(res => {
      ...
      const viewRowsSQL = isExitViewTableWidget(normalizedPageContent) ? generatorSearchSQL(activeTable, view, 0, 10000) : `SELECT * FROM \`${activeTable.name}\` where \`_id\` = '${row_id}'`;
      window.dtableAPIGateway.sqlQuery(dtable_uuid, viewRowsSQL, [], false).then(res => {
        const viewRows = res.data.results || [];
        const row = viewRows.find(row => row._id === row_id);
        const printRows = [row];
        if (linkWidgets.length > 0) {
          ...
          window.dtableWebAPI.pageDesignQueryRowLinkRecords(dtable_uuid, table_id, row_id, JSON.stringify(linkColumns)).then(res => {
            this.setState({
              ...
              printRows,
              viewRows,
              linkRows: { [row_id]: res.data.link_records },
              value,
            });
          }).catch(error => {
  ...
  render() {
    const { pageContent, activeTable, printRows, error, linkRows, collaborators, value, activeView,
      viewRows } = this.state;
