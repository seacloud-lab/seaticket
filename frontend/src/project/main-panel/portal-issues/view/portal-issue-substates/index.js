...
import { useMetadata } from '@/project/main-panel/tickets/hooks';
...
const AllPortalIssueSubstates = ({ projectUuid, permission }) => {
  ...
  const { pageSlugId, togglePageSlugId } = usePortalIssuesPage();
  ...
  const localStorageName = useMemo(() => `sea-qa-${projectUuid}-portal-issue-substates`, [projectUuid]);
  ...
  useEffect(() => {
    const unsubscribeNewSubstate = eventBus.subscribe(EVENT_BUS_TYPE.NEW_SUBSTATE, () => {
      context.eventBus.dispatch('expand_row');
    });
    return () => {
      unsubscribeNewSubstate();
    };
  }, []);
  ...
  return (
    <>
      <SeaMetadata
        ...
        t={t}
      >
        <OptionDialog type={gettext('substate')} canModifyDescription={true} parentOptions={statesData.rows} />
      </SeaMetadata>
    </>
  );
};

export default AllPortalIssueSubstates;
