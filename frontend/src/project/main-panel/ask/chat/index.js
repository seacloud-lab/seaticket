...
const Chat = ({ isShowSessions, sessionId, projectUuid, settings, projectName, workspaceID }) => {
  ...
  const sendMessage = useCallback(({ resolveType, message, tickets, issues, model }) => {
    ...
    if (sessionId !== ASK_PAGE_SLUG_ID.NEW) {
      eventBus.dispatch(EVENT_BUS_TYPE.ASK_QUESTION, { sessionId, message: validMessage, resolveType, tickets, issues, model });
      return;
    }
    createSession(validMessage.slice(0, 100)).then(session => {
      ...
      setTimeout(() => {
        eventBus.dispatch(EVENT_BUS_TYPE.ASK_QUESTION, { sessionId: newSessionId, message: validMessage, resolveType, tickets, issues, model });
      }, 3);
    });
