...
function getCity(province, string, geoTree) {
  let city; let startPoint = 0; let endPoint = 2;
  if (string.length === 0) {
    return { province, city: '', district: '', detail: '' };
  }
  ...
  if (province) {
    ...
    while (subCity.length === 2) {
      const newSubCity = subCity;
      let selectedCity = province.children.find((cityItem) => {
        if (cityItem.name.indexOf(newSubCity) === 0) {
          return true;
        } else {
          return false;
        }
  ...
  if (!province) {
    let city; let newProvince;
    const geoTreeChildren = geoTree.children;
    while (subCity.length === 2) {
      const newSubCity = subCity;
      for (let i = 0; i < geoTreeChildren.length; i++) {
        const provinceItem = geoTreeChildren[i];
        let selectedCity = provinceItem.children.find((city) => {
          return city.name.indexOf(newSubCity) === 0;
        });
        if (selectedCity) {
          city = selectedCity;
    ...
    if (city) {
      while (subDistrict.length === 2) {
        ...
        let selectedDistrict = city.children.find((item) => {
          return item.name.indexOf(newSubDistrict) === 0;
        });
        if (selectedDistrict) {
...
const parseGeolocationString = (geoTree, geoString) => {
  if (geoString.length < 2) {
    return { province: null, city: null, district: null, detail: geoString };
  }
