...
import { toaster, ModalHeader, PasswordInput } from '@/components';
...
class UserResetPassword extends React.Component {
  ...
  onSendCode = (e) => {
    ...
    profileSettingsAPI.sendVerifyCode(bindPhone, 'reset_password').then((res) => {
      this.setState({
  ...
  resetPassword = () => {
    ...
    profileSettingsAPI.resetPasswordByPhone(bindPhone, inputVerifyCode, newPassword, confirmPassword).then((res) => {
      if (this.timer) {
        clearInterval(this.timer);
      }
      // reset
