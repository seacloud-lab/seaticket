...
class Item extends Component {
  ...
  onDeleteSession = () => {
    ...
    profileSettingsAPI.deleteSession(session_id).then(() => {
      ...
      <Fragment>
        ...
        {this.state.isLogOutDialogOpen && (
          <CommonOperationConfirmationDialog
            ...
            message={gettext('Are you sure to log out {user}?').replace('{user}', `<b>${item.user_name}</b>`)}
            confirmBtnText={gettext('Log out')}
            toggleDialog={this.toggleLogOutDialog}
            executeOperation={this.onLogOutSession}
          />
        )}
      </Fragment>
    );
...
class LoggedInSessions extends Component {
  ...
  listSessions = () => {
    profileSettingsAPI.listSessions().then((res) => {
      this.setState({
        loading: false,
