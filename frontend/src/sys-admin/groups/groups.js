...
class Groups extends Component {
  ...
  getGroupListByPage = (page) => {
    sysAdminAPI.sysAdminListAllGroups(page, this.state.perPage).then((res) => {
      this.setState({
        loading: false,
  ...
  createGroup = (groupName, OnwerEmail) => {
    sysAdminAPI.sysAdminCreateNewGroup(groupName, OnwerEmail).then(res => {
  ...
  transferGroup = (groupID, receiverEmail) => {
    sysAdminAPI.sysAdminTransferGroup(receiverEmail, groupID).then(res => {
  ...
  downloadGroupExcel = () => {
    let url = `${siteRoot}sys/groups/export-excel/`;
    location.href = url;
  };
    ...
    if (isDesktop) {
      MainPanelTopbarContainer = (
        <MainPanelTopbar search={this.getSearch()}>
          ...
          <a className="btn btn-secondary operation-item" href={`${siteRoot}sys/groups/export-excel/`}>{gettext('Export Excel')}</a>
        </MainPanelTopbar>
      );
    } else {
      MainPanelTopbarContainer = (
