...
import { ActiveStatusEditor, toaster, Loading, Paginator, EmptyTip, CommonOperationConfirmationDialog
} from '@/components';
import SelectEditor from '@/components/select-editor';
import SysAdminAddSysUserNotificationDialog from '@/sys-admin/dialog/sysadmin-add-sys-user-notification-dialog';
import SysAdminUserSetQuotaDialog from '@/sys-admin/dialog/set-quota';
...
class Item extends Component {
  ...
  addSysUserNotification = (msg, email) => {
    sysAdminAPI.sysAdminAddSysUserNotification(msg, email).then(res => {
...
class UsersAll extends Component {
  ...
  getUserList = () => {
  // get admins
    sysAdminAPI.sysAdminListAdmins().then(res => {
  ...
  getUsersListByPage = (page) => {
    ...
    sysAdminAPI.sysAdminListUsers(page, perPage, this.props.isLDAPImported).then(res => {
  ...
  downloadUserExcel = () => {
    const url = `${siteRoot}sys/users/export-excel/`;
    location.href = url;
  };
  ...
  getOperationsForAll = () => {
    ...
    // if (isLDAPImported) {
    //   return <a className="btn btn-secondary operation-item" href={`${siteRoot}sys/users/export-excel/`}>{gettext('Export Excel')}</a>;
    // }
    ...
    return (
      <Fragment>
        ...
        <a className="btn btn-secondary operation-item" href={`${siteRoot}sys/users/export-excel/`}>{gettext('Export Excel')}</a>
      </Fragment>
    );
  };
