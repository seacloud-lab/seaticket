...
const Body = ({ isLoading, emptyTip, columns = [], rows = [], loadMore, onDelete, onModify, showStatus, onManualCrawl }) => {
        ...
        return (
          <div className="sea-custom-table-row" key={row.id}>
            {columns.map(column => {
              ...
              const valueFormatter = isValidElement(formatter) && cloneElement(formatter, { value, column, row, onModify, onDelete, showStatus, onManualCrawl });
              return (
