...
class SetOrgUserContactEmail extends React.Component {
  ...
  formSubmit = () => {
    ...
    orgAdminServiceApi.orgAdminSetOrgUserContactEmail(orgID, email, contactEmail).then((res) => {
  ...
  render() {
    ...
    return (
      <Modal isOpen={true} centered={true} toggle={this.props.toggleDialog}>
        <DTableModalHeader toggle={this.props.toggleDialog}>{gettext('Set user contact email')}</DTableModalHeader>
        <ModalBody>
          <React.Fragment>
