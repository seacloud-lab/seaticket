...
class OrgAdminShareTableToUser extends React.Component {
  ...
  componentDidMount() {
    orgAdminServiceApi.orgAdminListTableShares(orgID, this.dtableUuid).then((res) => {
      this.setState({ userList: res.data.user_shares });
    }).catch(error => {
  ...
  addTableShare = () => {
    ...
    for (let i = 0; i < selectedOptions.length; i++) {
      ...
      orgAdminServiceApi.orgAdminAddTableUserShare(orgID, this.dtableUuid, email, permission).then((res) => {
        let userInfo = {
          name: name,
          email: email,
  ...
  deleteTableShare = (email) => {
    ...
    orgAdminServiceApi.orgAdminDeleteTableUserShare(orgID, this.dtableUuid, email).then((res) => {
  ...
  updateTableShare = (email, permission) => {
    orgAdminServiceApi.orgAdminUpdateTableUserShare(orgID, this.dtableUuid, email, permission).then((res) => {
...
OrgAdminShareTableToUser.propTypes = propTypes;

export default OrgAdminShareTableToUser;
