...
def test_parse_suggested_label_content_uses_structured_labels():
    labels = AgentActionExecutor._parse_suggested_label_content(
        '["Bug", "needs-info", "bug", "  "]'
    )
...
def test_parse_suggested_label_content_rejects_non_list_json():
    assert AgentActionExecutor._parse_suggested_label_content('{"label": "Bug"}') == []


def test_parse_suggested_label_content_rejects_invalid_json():
    assert AgentActionExecutor._parse_suggested_label_content('Bug') == []
...
def test_github_label_action_passes_structured_content_to_executor():
