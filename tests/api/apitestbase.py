    ...
    def _req(cls, method, *args, **kwargs):
        ...
        if expected is not None:
            if hasattr(expected, '__iter__'):
                unittest.TestCase().assertIn(resp.status_code, expected,
                    "Expected http status in %s, received %s" % (expected,
                        resp.status_code))
            else:
                unittest.TestCase().assertEqual(resp.status_code, expected,
...
def get_auth_token(username, password):
    ...
    unittest.TestCase().assertEqual(res.status_code, 200)
    token = res.json()['token']
    unittest.TestCase().assertEqual(len(token), 40)
    return token
