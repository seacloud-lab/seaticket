...
from seahub.utils.storage import upload_portal_background_image_file_to_s3, upload_portal_logo_file_to_s3
...
def test_upload_portal_background_image_file_to_s3_uses_project_path(tmp_path, monkeypatch):
    ...
    assert called["local_path"] == str(tmp_upload_file)
    assert called["key"] == f"/projects/{project_uuid}/attachments/portal-background-image/background-image"
    assert called["extra_args"] == {"ContentType": "image/png"}
