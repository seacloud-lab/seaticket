...
class TestPortalTicketsView:
    ...
    def test_post_missing_title(self, factory, auth_user):
    ...
    def test_post_invalid_content_json(self, factory, auth_user):
...
class TestPortalMyTicketsView:
    ...
    def test_post_start_invalid(self, factory, auth_user):
...
class TestPortalTicketTypesView:
    ...
    def test_get_success(self, factory, auth_user, real_project):
        project = real_project
        request = factory.get(f"/api/v1/portal/{project.uuid}/ticket/types/")
        request.user = auth_user
        ...
        with patch('seahub.utils.decorators.is_org_context', return_value=True), \
            ...
            resp = PortalTicketTypesView.as_view()(request, project_uuid=str(project.uuid))

        assert resp.status_code == 200
        assert resp.data['tags'][0]['name'] == 't1'
