...
class TestLoadVisitorSession:
    ...
    def test_expired_session_returns_expired(self, factory):
        ...
        cache.clear()
        request = factory.get('/')
        signer_cookie = _get_signed_cookie_for(session_data['visitor_id'])
    ...
    def test_active_session_returns_active(self, factory):
        ...
        assert result['visitor_id'] == session_data['visitor_id']
...
class TestTouchVisitorSession:
    ...
    def test_updates_last_seen_at(self):
        ...
        updated = touch_visitor_session(session_data['visitor_id'], session_data)
    ...
    def test_refresh_cookie_updates_last_refreshed_at(self):
        ...
        updated = touch_visitor_session(
            session_data['visitor_id'], session_data, refresh_cookie=True
        )
...
class TestCookieOperations:
    ...
    def test_ensure_visitor_cookie_refreshes_existing(self, factory):
        ...
        cached = cache.get(
            ...
            session_data['visitor_id'].replace('-', '')
        )
        assert cached is not None


def _get_signed_cookie_for(visitor_id):
    from seahub.portal.visitor_session import _sign_visitor_id
    return _sign_visitor_id(visitor_id)
