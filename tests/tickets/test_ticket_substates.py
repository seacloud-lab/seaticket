...
def test_get_substates_success(factory, auth_user, real_project):
...
def test_get_substates_filter_by_state_id(factory, auth_user, real_project):
    project = real_project
...
def test_delete_substates_success(factory, auth_user, real_project):
    project = real_project
    request = factory.delete(
...
def test_post_substate_duplicate_name(factory, auth_user, real_project):
    project = real_project
...
def test_post_substate_success(factory, auth_user, real_project):
    project = real_project
    payload = {'name': 'sub1', 'color': '#fff', 'text_color': '#000', 'parent_id': 'st1'}
    request = factory.post(f"/api/v1/projects/{project.uuid}/ticket-substates/", data=payload)
    request.user = auth_user
...
def test_get_substate_not_found(factory, auth_user, real_project):
    project = real_project
    ...
    with patch('seahub.utils.decorators.is_org_context', return_value=True), \
            ...
            patch('seahub.tickets.ticket_substates.get_current_table_metadata', return_value=table_meta), \
            patch('seahub.tickets.ticket_substates.get_column_from_columns_by_name', return_value=column):
        resp = TicketSubstateAPIView.as_view()(request, project_uuid=str(project.uuid), substate_id='s1')
...
def test_delete_substate_not_found(factory, auth_user, real_project):
    project = real_project
...
def test_get_substate_success(factory, auth_user, real_project):
    project = real_project
    request = factory.get(f"/api/v1/projects/{project.uuid}/ticket-substates/s1/")
    request.user = auth_user
