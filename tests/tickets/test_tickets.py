...
class TestTicketsAPIView:
    ...
    def test_get_non_int_start_limit_use_default(self, factory, user):
        ...
        with mock_project_and_permission(user), \
                ...
                patch('seahub.project.seadb_api.SeaDBAPI', return_value=Mock()), \
    ...
    def test_get_success(self, factory, user):
        ...
        with mock_project_and_permission(user), \
                ...
                patch('seahub.project.seadb_api.SeaDBAPI') as seadb_cls_mock, \
                patch('seahub.tickets.tickets.list_tickets_view_records') as list_mock:
...
class TestTicketAPIView:
    def test_get_ticket_not_found(self, factory, user):
        ...
        with mock_project_and_permission(user), \
                ...
                patch('seahub.tickets.ticket_utils.get_ticket', return_value=(None, None)):
            seadb_cls_mock.return_value = Mock()
    ...
    def test_get_success(self, factory, user):
        ...
        with mock_project_and_permission(user), \
                patch('seahub.project.seadb_api.SeaDBAPI') as seadb_cls_mock, \
                patch('seahub.tickets.ticket_utils.get_ticket', return_value=(ticket, metadata)), \
    ...
    def test_put_permission_denied(self, factory, user):
        ...
        with mock_project_and_permission(user), \
                patch('seahub.project.seadb_api.SeaDBAPI', return_value=Mock()), \
                patch('seahub.tickets.ticket_utils.get_ticket', return_value=(ticket, {})), \
                patch('seahub.project.utils.check_ticket_permission', return_value=False):
            resp = TicketAPIView.as_view()(request, project_uuid='p1', ticket_id='1')
...
class TestTicketCommentAPIView:
    def test_put_missing_content(self, factory, user):
        ...
        request.user = user
        ticket = {'_pk': 1}
        ticket_comment = {'_pk': 2}
        with mock_project_and_permission(user), \
                patch('seahub.project.seadb_api.SeaDBAPI', return_value=Mock()), \
                patch('seahub.tickets.ticket_utils.get_ticket', return_value=(ticket, {})), \
                patch('seahub.tickets.ticket_utils.get_ticket_comment_by_pk', return_value=ticket_comment), \
                patch('seahub.project.utils.check_comment_permission', return_value=True):
    ...
    def test_put_comment_not_found(self, factory, user):
        ...
        with mock_project_and_permission(user), \
                patch('seahub.project.seadb_api.SeaDBAPI', return_value=Mock()), \
                patch('seahub.tickets.ticket_utils.get_ticket', return_value=(ticket, {})), \
                patch('seahub.tickets.ticket_utils.get_ticket_comment_by_pk', return_value=None):
            resp = TicketCommentAPIView.as_view()(request, project_uuid='p1', ticket_id='1', comment_id='1')
        assert resp.status_code == 404
